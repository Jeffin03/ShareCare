 ...
 * scripts/seed-dev.js
 *
 * Development seed script for sharecare-dev.
 * Uses a local service account JSON file — never committed to git.
 *
 * Setup (one-time):
 *   1. Go to Firebase Console → Project Settings → Service Accounts
 *   2. Make sure you are on the sharecare-dev project
 *   3. Click "Generate new private key" and download the JSON
 *   4. Save it as: scripts/serviceAccount.dev.json
...
const SERVICE_ACCOUNT_PATH = path.join(__dirname, 'serviceAccount.dev.json');
...
if (!fs.existsSync(SERVICE_ACCOUNT_PATH)) {
  ...
  console.error('       1. Open Firebase Console and switch to the sharecare-dev project.');
  console.error('       2. Go to Project Settings → Service Accounts.');
  console.error('       3. Click "Generate new private key" and download the JSON.');
  console.error('       4. Save it as scripts/serviceAccount.dev.json');
  console.error('');
  process.exit(1);
}
...
// Quick sanity check — make sure the key is for the dev project
if (serviceAccount.project_id !== 'sharecare-dev') {
  console.error('\n❌  Wrong service account!');
  console.error(`     Expected project_id: sharecare-dev`);
  console.error(`     Got project_id:      ${serviceAccount.project_id}`);
  console.error('');
  console.error('     Make sure you downloaded the key from the sharecare-dev project.');
  process.exit(1);
}
...
async function seed() {
  console.log('');
  console.log('╔══════════════════════════════════════════════════╗');
  console.log('║     ShareCare — Dev Seed Script                  ║');
  console.log(`║     Project: ${'sharecare-dev'.padEnd(36)}║`);
  console.log('╚══════════════════════════════════════════════════╝');
